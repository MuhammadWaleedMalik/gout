...
const Donate: React.FC = () => {
  ...
  const pageContent: DonateContent = languageMap[currentLanguage?.code as keyof typeof languageMap] || languageMap.en;
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    setFormData({ churchName: '', amount: '', paymentMethod: '' });
  };
        ...
        <div className="max-w-4xl mx-auto">
          ...
          >
            ...
            <h2 className="text-2xl font-bold mb-4" style={{ color: colors.primaryColor1 }}>
              {processText(pageContent.donate.title)}
            </h2>
            <p className="text-base text-gray-600 max-w-2xl mx-auto mb-8">
              {processText(pageContent.donate.description)}
            </p>
            <motion.div
              initial={{ opacity: 0, y: 20 }}
              whileInView={{ opacity: 1, y: 0 }}
            ...
            >
              <h3 className="text-xl font-bold mb-3" style={{ color: colors.primaryColor1 }}>
                {processText(pageContent.donate.form.title)}
              </h3>
              <p className="text-gray-600 mb-6">{processText(pageContent.donate.form.subtitle)}</p>
              <div className="space-y-4" onSubmit={handleSubmit}>
                <div>
                  <label className="block text-gray-700 mb-1" htmlFor="churchName">
                    {processText(pageContent.donate.form.fields.churchName)}
                  </label>
                  <input
                    type="text"
                    id="churchName"
                ...
                <div>
                  <label className="block text-gray-700 mb-1" htmlFor="amount">
                    {processText(pageContent.donate.form.fields.amount)}
                  </label>
                  <input
                    type="number"
                    id="amount"
                ...
                <div>
                  <label className="block text-gray-700 mb-1" htmlFor="paymentMethod">
                    {processText(pageContent.donate.form.fields.paymentMethod)}
                  </label>
                  <select
                    id="paymentMethod"
                    name="paymentMethod"
                  ...
                  >
                    ...
                    {pageContent.donate.form.paymentMethods.map((method) => (
                ...
                >
                  {processText(pageContent.donate.form.fields.submit)}
                </button>
              </div>
            </motion.div>
          </motion.div>
